...
sub new {
  ...
  if (! ($self->opts->hostname || $self->opts->snmpwalk)) {
    ...
    if (! $UPS::Device::plugin->check_messages()) {
  ...
  $UPS::Device::statefilesdir = $self->opts->statefilesdir;
  return $self;
}
...
sub init {
  ...
  if ($self->mode =~ /device::walk/) {
    ...
    if (defined $self->opts->offline) {
      ...
      $SIG{'ALRM'} = sub {
        $timedout = 1;
        printf "UNKNOWN - check_ups_health timed out after %d seconds\n",
            $self->opts->timeout;
        kill 9, $snmpwalkpid;
      };
...
sub whoami {
  ...
  if ($productname = $self->get_snmp_object('MIB-II', 'sysDescr', 0)) {
    ...
    $UPS::Device::uptime = $self->{uptime};
  } else {
    $self->add_message(CRITICAL,
        'snmpwalk returns no product name (sysDescr)');
    if (! $self->opts->snmpwalk) {
      $UPS::Device::session->close;
    }
  }
...
sub get_snmp_object {
  ...
  my $mo = shift;
  my $index = shift;
  if (exists $UPS::Device::mibs_and_oids->{$mib} &&
      exists $UPS::Device::mibs_and_oids->{$mib}->{$mo}) {
    my $oid = $UPS::Device::mibs_and_oids->{$mib}->{$mo}.
        (defined $index ? '.'.$index : '');
...
sub get_request_iq {
  ...
  foreach my $oid (@{$params{'-molist'}}) {
    if (exists $UPS::Device::mibs_and_oids->{$mib} &&
        exists $UPS::Device::mibs_and_oids->{$mib}->{$oid}) {
      push(@oids, (exists $params{'-index'}) ?
          $UPS::Device::mibs_and_oids->{$mib}->{$oid}.'.'.$params{'-index'} :
          $UPS::Device::mibs_and_oids->{$mib}->{$oid});
    }
  }
...
sub valid_response {
  ...
  if (exists $UPS::Device::mibs_and_oids->{$mib} &&
      exists $UPS::Device::mibs_and_oids->{$mib}->{$oid}) {
    # make it numerical
    my $oid = $UPS::Device::mibs_and_oids->{$mib}->{$oid};
    if (defined $index) {
      $oid .= '.'.$index;
    }
...
sub debug {
  my $self = shift;
  my $format = shift;
  $self->{trace} = -f "/tmp/check_ups_health.trace" ? 1 : 0;
  ...
  if ($self->{trace}) {
    ...
    $logfh->autoflush(1);
    if ($logfh->open("/tmp/check_ups_health.trace", "a")) {
...
sub mode {
  my $self = shift;
  return $UPS::Device::mode;
}

sub uptime {
  my $self = shift;
  return $UPS::Device::uptime;
}
...
sub add_message {
  ...
  my $level = shift;
  my $message = shift;
  $UPS::Device::plugin->add_message($level, $message) 
...
sub status_code {
  my $self = shift;
  return $UPS::Device::plugin->status_code(@_);
}

sub check_messages {
  my $self = shift;
  return $UPS::Device::plugin->check_messages(@_);
}

sub clear_messages {
  my $self = shift;
  return $UPS::Device::plugin->clear_messages(@_);
}

sub suppress_messages {
  my $self = shift;
  return $UPS::Device::plugin->suppress_messages(@_);
}

sub add_perfdata {
  my $self = shift;
  $UPS::Device::plugin->add_perfdata(@_);
}

sub set_thresholds {
  my $self = shift;
  $UPS::Device::plugin->set_thresholds(@_);
}

sub force_thresholds {
  my $self = shift;
  $UPS::Device::plugin->force_thresholds(@_);
}
...
sub check_thresholds {
  ...
  ($self->{warning}, $self->{critical}) =
      $UPS::Device::plugin->get_thresholds(@params);
  return $UPS::Device::plugin->check_thresholds(@params);
}

sub get_thresholds {
  my $self = shift;
  my @params = @_;
  my @thresholds = $UPS::Device::plugin->get_thresholds(@params);
  my($warning, $critical) = $UPS::Device::plugin->get_thresholds(@params);
...
sub add_info {
  ...
  $self->{info} = $info;
  push(@{$UPS::Device::info}, $info);
}

sub annotate_info {
  my $self = shift;
  my $annotation = shift;
  my $lastinfo = pop(@{$UPS::Device::info});
  $lastinfo .= sprintf ' (%s)', $annotation;
  push(@{$UPS::Device::info}, $lastinfo);
}
...
sub add_extendedinfo {
  ...
  push(@{$UPS::Device::extendedinfo}, $info);
}

sub get_extendedinfo {
  my $self = shift;
  return join(' ', @{$UPS::Device::extendedinfo});
}

sub add_summary {
  my $self = shift;
  my $summary = shift;
  push(@{$UPS::Device::summary}, $summary);
}

sub get_summary {
  my $self = shift;
  return join(', ', @{$UPS::Device::summary});
}

sub opts {
  my $self = shift;
  return $UPS::Device::plugin->opts();
}

sub set_rawdata {
  my $self = shift;
  $UPS::Device::rawdata = shift;
}
...
sub add_rawdata {
  ...
  my $oid = shift;
  my $value = shift;
  $UPS::Device::rawdata->{$oid} = $value;
}

sub rawdata {
  my $self = shift;
  return $UPS::Device::rawdata;
}
...
sub get_snmp_table_attributes {
  ...
  if (exists $UPS::Device::mibs_and_oids->{$mib} &&
      exists $UPS::Device::mibs_and_oids->{$mib}->{$table}) {
    my $toid = $UPS::Device::mibs_and_oids->{$mib}->{$table}.'.';
    my $toidlen = length($toid);
    my @columns = grep {
      substr($UPS::Device::mibs_and_oids->{$mib}->{$_}, 0, $toidlen) eq
          $UPS::Device::mibs_and_oids->{$mib}->{$table}.'.'
    } keys %{$UPS::Device::mibs_and_oids->{$mib}};
    if ($augmenting_table &&
        exists $UPS::Device::mibs_and_oids->{$mib}->{$augmenting_table}) {
      my $toid = $UPS::Device::mibs_and_oids->{$mib}->{$augmenting_table}.'.';
      my $toidlen = length($toid);
      push(@columns, grep {
        substr($UPS::Device::mibs_and_oids->{$mib}->{$_}, 0, $toidlen) eq
            $UPS::Device::mibs_and_oids->{$mib}->{$augmenting_table}.'.'
      } keys %{$UPS::Device::mibs_and_oids->{$mib}});
    }
    return @columns;
  } else {
...
sub get_request {
  ...
  foreach my $oid (@{$params{'-varbindlist'}}) {
    ...
    if (! exists UPS::Device::rawdata->{$oid}) {
      push(@notcached, $oid);
    }
  }
  if (! $self->opts->snmpwalk && (scalar(@notcached) > 0)) {
    my $result = ($UPS::Device::session->version() == 0) ?
        $UPS::Device::session->get_request(
            -varbindlist => \@notcached,
        )
        :
        $UPS::Device::session->get_request(  # get_bulk_request liefert next
  ...
  my $result = {};
  map { $result->{$_} = $UPS::Device::rawdata->{$_} }
      @{$params{'-varbindlist'}};
  return $result;
}
...
sub get_snmp_table_objects {
  ...
  if (scalar(@{$indices}) == 1) {
    if (exists $UPS::Device::mibs_and_oids->{$mib} &&
        exists $UPS::Device::mibs_and_oids->{$mib}->{$table}) {
      my $eoid = $UPS::Device::mibs_and_oids->{$mib}->{$entry}.'.';
      my $eoidlen = length($eoid);
      my @columns = map {
          $UPS::Device::mibs_and_oids->{$mib}->{$_}
      } grep {
        substr($UPS::Device::mibs_and_oids->{$mib}->{$_}, 0, $eoidlen) eq
            $UPS::Device::mibs_and_oids->{$mib}->{$entry}.'.'
      } keys %{$UPS::Device::mibs_and_oids->{$mib}};
      ...
      if ($augmenting_table && 
        ...
        my $eoid = $UPS::Device::mibs_and_oids->{$mib}->{$augmenting_entry}.'.';
        my $eoidlen = length($eoid);
        push(@columns, map {
            $UPS::Device::mibs_and_oids->{$mib}->{$_}
        } grep {
    ...
    if (exists $UPS::Device::mibs_and_oids->{$mib} &&
      ...
      #my @indices =
      # $self->get_indices($UPS::Device::mibs_and_oids->{$mib}->{$entry});
      @entries = $self->make_symbolic($mib, $result, $indices);
      @entries = map { $_->{indices} = shift @{$indices}; $_ } @entries;
    }
  } else {
    if (exists $UPS::Device::mibs_and_oids->{$mib} &&
      ...
      my $result = $self->get_table(
          -baseoid => $UPS::Device::mibs_and_oids->{$mib}->{$table});
      ...
      my @indices = 
          $self->get_indices(
              -baseoid => $UPS::Device::mibs_and_oids->{$mib}->{$entry},
              -oids => [keys %{$result}]);
        ...
        if (exists $result->{$fulloid}) {
          if (exists $UPS::Device::mibs_and_oids->{$mib}->{$symoid.'Definition'}) {
            if (ref($UPS::Device::mibs_and_oids->{$mib}->{$symoid.'Definition'}) eq 'HASH') {
              if (exists $UPS::Device::mibs_and_oids->{$mib}->{$symoid.'Definition'}->{$result->{$fulloid}}) {
                $mo->{$symoid} = $UPS::Device::mibs_and_oids->{$mib}->{$symoid.'Definition'}->{$result->{$fulloid}};
              } else {
                $mo->{$symoid} = 'unknown_'.$result->{$fulloid};
              }
            } elsif ($UPS::Device::mibs_and_oids->{$mib}->{$symoid.'Definition'} =~ /^(.*?)::(.*)/) {
              my $mib = $1;
              my $definition = $2;
              if  (exists $UPS::Device::definitions->{$mib} && exists $UPS::Device::definitions->{$mib}->{$definition}
                  && exists $UPS::Device::definitions->{$mib}->{$definition}->{$result->{$fulloid}}) {
                $mo->{$symoid} = $UPS::Device::definitions->{$mib}->{$definition}->{$result->{$fulloid}};
              } else {
                $mo->{$symoid} = 'unknown_'.$result->{$fulloid};
              }
            } else {
              $mo->{$symoid} = 'unknown_'.$result->{$fulloid};
              # oder $UPS::Device::mibs_and_oids->{$mib}->{$symoid.'Definition'}?
            }
          } else {
          ...
          if (exists $UPS::Device::mibs_and_oids->{$mib}->{$symoid.'Definition'}) {
            if (ref($UPS::Device::mibs_and_oids->{$mib}->{$symoid.'Definition'}) eq 'HASH') {
              if (exists $UPS::Device::mibs_and_oids->{$mib}->{$symoid.'Definition'}->{$result->{$oid}}) {
                $mo->{$symoid} = $UPS::Device::mibs_and_oids->{$mib}->{$symoid.'Definition'}->{$result->{$oid}};
                push(@entries, $mo);
              }
            } elsif ($UPS::Device::mibs_and_oids->{$mib}->{$symoid.'Definition'} =~ /^(.*?)::(.*)/) {
              my $mib = $1;
              my $definition = $2;
              if  (exists $UPS::Device::definitions->{$mib} && exists $UPS::Device::definitions->{$mib}->{$definition}
                  && exists $UPS::Device::definitions->{$mib}->{$definition}->{$result->{$oid}}) {
                $mo->{$symoid} = $UPS::Device::definitions->{$mib}->{$definition}->{$result->{$oid}};
              } else {
...
sub get_entries {
  ...
  if (! $self->opts->snmpwalk) {
    ...
    $result = $UPS::Device::session->get_entries(%newparams);
    if (! $result) {
...
sub get_matching_oids {
  ...
  foreach my $oid (@{$params{'-columns'}}) {
    ...
    $oidpattern =~ s/\./\\./g;
    map { $result->{$_} = $UPS::Device::rawdata->{$_} }
        grep /^$oidpattern(?=\.|$)/, keys %{$UPS::Device::rawdata};
  }
  $self->debug(sprintf "get_matching_oids returns %d from %d oids", 
      scalar(keys %{$result}), scalar(keys %{$UPS::Device::rawdata}));
  return $result;
}
...
sub create_statefilesdir {
  my $self = shift;
  if (! -d $UPS::Device::statefilesdir) {
    eval {
      use File::Path;
      mkpath $UPS::Device::statefilesdir;
    };
    if ($@ || ! -w $UPS::Device::statefilesdir) {
      $self->add_message(UNKNOWN,
        sprintf "cannot create status dir %s! check your filesystem (permissions/usage/integrity) and disk devices", $UPS::Device::statefilesdir);
    }
  } elsif (! -w $UPS::Device::statefilesdir) {
    $self->add_message(UNKNOWN,
        sprintf "cannot write status dir %s! check your filesystem (permissions/usage/integrity) and disk devices", $UPS::Device::statefilesdir);
  }
}
...
sub create_statefile {
  ...
  if ($self->opts->snmpwalk && ! $self->opts->hostname) {
    return sprintf "%s/%s_%s%s", $UPS::Device::statefilesdir,
...
sub create_interface_cache_file {
  ...
  $extension =~ s/\*/_/g;
  $extension =~ s/\s/_/g;
  return sprintf "%s/%s_interface_cache_%s", $UPS::Device::statefilesdir,
...
sub get_table_entries {
  ...
  if (exists $UPS::Device::mibs_and_oids->{$mib} &&
      exists $UPS::Device::mibs_and_oids->{$mib}->{$table}) {
    foreach my $key (keys %{$UPS::Device::mibs_and_oids->{$mib}}) {
      if ($UPS::Device::mibs_and_oids->{$mib}->{$key} =~
          /^$UPS::Device::mibs_and_oids->{$mib}->{$table}/) {
        $oids->{$key} = $UPS::Device::mibs_and_oids->{$mib}->{$key};
      }
    }
  }
...
sub update_entry_cache {
  ...
  my $statefile = lc sprintf "%s/%s_%s_%s-%s_%s_cache",
      $UPS::Device::statefilesdir, $self->opts->hostname,
...
define service {
  ...
  check_command                 check_ups_health!cpu-load!80%%!90%%
}
EOEO
  printf <<'EOEO', $self->opts->hostname();
define service {
  ...
  check_command                 check_ups_health!memory-usage!80%%!90%%
}
EOEO
  printf <<'EOEO', $self->opts->hostname();
define service {
  ...
  check_command                 check_ups_health!interface-usage!$VALUE1$!$VALUE2$
